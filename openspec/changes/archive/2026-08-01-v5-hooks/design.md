...
**Non-Goals:** **Stop hook（本 change 推迟）**、Compact hooks、SessionStart 全家桶、Agent hooks、完整 CC settings 兼容。
...
Stop hook **不在本 change 实现**。若后续需要：模型 end_turn 时跑 Stop；要求继续则注入合成 user 再进一轮。
